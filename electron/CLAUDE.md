# Electron — main process, preload, shared

Read with the root `CLAUDE.md`. This covers what only matters when working in `electron/`.

## Process model

- The preload (`preload/`) is sandboxed and builds to CommonJS (`.cjs`). It is the only
  bridge: narrow, typed methods on `window.mosaic`, never raw IPC or Node. Channel names and
  the `MosaicDb` method list live in `shared/`, used by both sides.
...
- Handlers come in two parts: argument checks with no Electron import
  ...
  thin file that registers them on `ipcMain`.
...
- The document is a JSON blob in one column. Do not normalize sections, entries, or bullets
  ...
  DDL would duplicate `src/types/resume.ts`. SQL indexes the _history_: `templates`,
...
- Erase (`main/eraseAll.ts`) forgets the API keys, then deletes the database file and
...
- Pre-v1: edit `main/db/migrations/001_init.sql` in place, then `bun run dev:reset`. From
...
- `migrate.ts` tracks progress with `PRAGMA user_version` and fingerprints each applied
...
- Releasing: `bun run db:freeze` records the shipping migrations in
  `migrations/released.json`. After that, `bun run test` fails if a released migration is
  edited — users' databases already ran it, so the change goes in a new file.

## API keys (`main/secrets/`)
...
- Test (`testKey.ts`) makes one request that costs no tokens. The key travels only in a
