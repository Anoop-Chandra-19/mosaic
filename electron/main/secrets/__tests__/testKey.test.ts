...
import { KEYED_PROVIDERS } from '@/types/secrets';
import { testKey, type KeyTestFetch } from '../testKey';
...
describe('testKey', () => {
  it('looks the model up, so a working key also says whether it reaches the model', async () => {
    ...
    expect(await testKey('openai', KEY, 'gpt-4.1-mini', found.fetch)).toEqual({
      ok: true,
      modelFound: true,
    });
    ...
    expect(await testKey('anthropic', KEY, 'claude-nope', missing.fetch)).toEqual({
      ok: true,
      modelFound: false,
    });
  });
  ...
  it('asks OpenRouter about the key itself, since its model list is public', async () => {
    ...
    expect(await testKey('openrouter', KEY, 'openai/gpt-4o-mini', fetch)).toEqual({
      ok: true,
      modelFound: null,
    });
    expect(sent[0].url).toBe('https://openrouter.ai/api/v1/key');
    // Its 404 is an error, not a missing model.
    expect(await testKey('openrouter', KEY, 'x', answering(404).fetch)).toEqual({
      ok: false,
      reason: 'failed',
      httpStatus: 404,
    });
  ...
  it('reports a refused key', async () => {
    for (const status of [401, 403]) {
      expect(await testKey('openai', KEY, 'gpt-4.1-mini', answering(status).fetch)).toEqual({
        ok: false,
        reason: 'refused',
        httpStatus: status,
      });
    ...
    expect(await testKey('gemini', KEY, 'gemini-2.0-flash', gemini.fetch)).toMatchObject({
      ok: false,
      reason: 'refused',
    });
  });
  ...
  it('reports other errors with their status, and no answer as unreachable', async () => {
    expect(await testKey('anthropic', KEY, 'm', answering(500).fetch)).toEqual({
      ok: false,
      reason: 'failed',
      httpStatus: 500,
    });
    expect(await testKey('gemini', KEY, 'm', answering(400, '{"error":"bad"}').fetch)).toEqual({
      ok: false,
      reason: 'failed',
      httpStatus: 400,
    });
    const offline: KeyTestFetch = () => Promise.reject(new TypeError('fetch failed'));
    expect(await testKey('openai', KEY, 'gpt-4.1-mini', offline)).toEqual({
      ok: false,
      reason: 'unreachable',
    });
  });
  ...
  it('sends the key in a header, never in the URL', async () => {
    for (const provider of KEYED_PROVIDERS) {
      const { fetch, sent } = answering(200);
      await testKey(provider, KEY, 'models/some-model', fetch);
  ...
  it('names the model safely in the URL', async () => {
    const { fetch, sent } = answering(200);
    await testKey('gemini', KEY, 'models/gemini-2.0-flash', fetch);
    await testKey('openai', KEY, 'ft:gpt-4o:acme::abc/../x', fetch);
