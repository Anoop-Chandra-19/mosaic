import { formatHeaderLineText, getPrintableHeaderLines } from '@shared/resume/resumeHeader';
...
function pageLines(doc: ResumeData): Map<string, string> {
  ...
  for (const section of [...doc.sections].sort((a, b) => a.order - b.order)) {
    ...
    for (const entry of section.items) {
      ...
      lines.set(
        `entry:${entry.id}`,
        [entry.title, entry.organization, entry.location, entry.dates, entry.text].join('|')
      );
