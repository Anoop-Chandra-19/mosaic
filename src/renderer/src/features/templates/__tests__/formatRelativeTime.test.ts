...
describe('formatRelativeTime', () => {
  it('says how long ago for today', () => {
    ...
    expect(formatRelativeTime(at(8, 13, 15, 18), now)).toBe('12 min ago');
    expect(formatRelativeTime(at(8, 13, 12, 10), now)).toBe('3h ago');
    expect(formatRelativeTime(at(8, 13, 0, 5), now)).toBe('15h ago');
  });

  it('names yesterday with the time', () => {
    expect(formatRelativeTime(at(8, 12, 21, 12), now)).toMatch(/^Yesterday, 9:12/);
  });

  it('gives the date for anything older, with the year only when it differs', () => {
    expect(formatRelativeTime(at(2, 4, 10), now)).toMatch(/Mar 4|4 Mar/);
    expect(formatRelativeTime(at(2, 4, 10, 0, 2025), now)).toMatch(/2025/);
  });
});
