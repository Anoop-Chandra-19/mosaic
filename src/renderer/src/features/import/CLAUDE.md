...
- Readers produce `ImportLine`s (`importLines.ts`); `parseResumeLines` builds the resume.
...
- In `pdf/` and `docx/`, `readPdf` / `readDocx` report what a file holds; `pdfLines` /
...
- Tests build fictional files (`buildDocx`, `buildPdf`, react-pdf); never use a real
  person's resume. `docx/__tests__/documents/` contains only fictional fixtures.
